...
const facebookRetryCounts=new Map();
const facebookRetryTimers=new Map();
...
function scheduleFacebookRetry(boardId,itemId){
  const key=boardId+':'+itemId;
  if(facebookRetryTimers.has(key))return;
  const count=facebookRetryCounts.get(key)||0;
  if(count>=4)return;

  const delays=[5000,12000,25000,45000];
  const timer=setTimeout(async()=>{
    facebookRetryTimers.delete(key);
    facebookRetryCounts.set(key,count+1);

    const b=projects.find(p=>p.id===boardId);
    const x=b?.items?.find(i=>i.id===itemId);
    if(!x||!isFacebookUrl(x.url)||x.image){
      facebookRetryCounts.delete(key);
      return;
    }
    ...
    scheduleFacebookRetry(boardId,itemId);
  },delays[count]);

  facebookRetryTimers.set(key,timer);
}
  ...
  try{
    ...
    for(let i=0;i<todo.length;i+=3){
      ...
      await Promise.all(batch.map(async x=>{
        ...
        if(isFacebookUrl(x.url)&&!x.image)scheduleFacebookRetry(boardId,x.id);
      }));
